...
void AssetManager_LoadTIMData(TIM_IMAGE *tim_data, const CdData *cd_data) {
    ...
    DrawSync(0);
    LOG_INFO(
        "Texture loaded from %s {x, y, w, h}=%d, %d, %d, %d",
        cd_data->name, tim_data->prect->x, tim_data->prect->y, tim_data->prect->w, tim_data->prect->h
    );
    ...
    if (tim_data->mode & 0x8) {
        ...
        DrawSync(0);
        LOG_INFO("CLUT added for %s {x, y}=%d, %d", cd_data->name, tim_data->crect->x, tim_data->crect->y);
    }
}
...
void AssetManager_TransferVAGToSPU(const CdData *cd_data, const u_long voice_channel) {
    ...
    if (vag_spu_addr == -1) {
        LOG_ERR("Could not allocate audio file %s, shutting down", cd_data->name);
        exit(1);
    }
